...
//Filter parameters (beta parameters) - Defaults tuned for 2kHz loop rate; Do not touch unless you know what you are doing:
float B_madgwick = 0.04;  //Madgwick filter parameter (default control 2kHz = 0.04, general default = 1.0)
...
float AccErrorY_main = 0.12;
float AccErrorZ_main = -0.00;
float GyroErrorX_main = -3.96;
float GyroErrorY_main = -2.43;
float GyroErrorZ_main = 0.35;
...
// Pin definitions for PWM and PPM receivers are now in quad.h (shared across all .ino files)
...
#else
int m1_command_PWM, m2_command_PWM, m3_command_PWM, m4_command_PWM, m5_command_PWM, m6_command_PWM; // Generic PWM
#endif
float s1_command_scaled, s2_command_scaled, s3_command_scaled, s4_command_scaled, s5_command_scaled, s6_command_scaled, s7_command_scaled;
int s1_command_PWM, s2_command_PWM, s3_command_PWM, s4_command_PWM, s5_command_PWM, s6_command_PWM, s7_command_PWM;
...
#else
  // Attach PWMServo objects for all motors/ESCs (generic ESCs)
  motor1_esc.attach(m1Pin, 1000, 2000); // Attach to motor 1 pin, with standard ESC PWM range
  motor2_esc.attach(m2Pin, 1000, 2000); // Attach to motor 2 pin
  motor3_esc.attach(m3Pin, 1000, 2000); // Attach to motor 3 pin
  motor4_esc.attach(m4Pin, 1000, 2000); // Attach to motor 4 pin
  motor5_esc.attach(m5Pin, 1000, 2000); // Attach to motor 5 pin
...
void loop() {
  ...
  // This allows onboard validation of control system perception during flight.
  ...
  #if defined USE_ELRS_RX
    // Send attitude telemetry at ~10Hz (every 100ms) to avoid overloading telemetry bandwidth
    static unsigned long lastTelemetryTime = 0;
    if (millis() - lastTelemetryTime >= 100) {
      elrs_rx.sendAttitude(roll_IMU, pitch_IMU, yaw_IMU);
...
void scaleCommands() {
...
#else
  //Scaled to 1000us - 2000us for standard PWM ESCs
  m1_command_PWM = m1_command_scaled*1000 + 1000; // From 0-1 scaled to 1000-2000
  ...
  //Constrain commands to motors within standard PWM bounds
...
void getCommands() {
  //DESCRIPTION: Get raw PWM values for every channel from the radio with configurable channel mapping
  /*
   * Updates radio PWM commands in loop based on current available commands. channel_x_pwm is the raw command used in the rest of 
   * the loop. Channel mapping is applied based on receiver type using defines from quad.h:
   * - PPM: PPM_MAP_CH1-6 maps logical channels to physical PPM slot numbers (1-based)
   * - PWM: PWM_MAP_CH1-6 maps logical channels to physical PWM pin channels (1-based)
   * - ELRS: ELRS_MAP_CH1-6 maps logical channels to physical CRSF channel indices (1-based)
   * - SBUS: SBUS_MAP_CH1-6 maps logical channels to physical SBUS channel array indices (0-based)
   ...
   */
   ...
   // PPM: Read physical channels and apply mapping (PPM_MAP_CH1-6 map logical to physical slot numbers)
    ...
    raw_channels[PPM_MAP_CH4] = getRadioPWM(PPM_MAP_CH4);
    raw_channels[PPM_MAP_CH5] = getRadioPWM(PPM_MAP_CH5);
    raw_channels[PPM_MAP_CH6] = getRadioPWM(PPM_MAP_CH6);
    
    // Map to logical channels
    channel_1_pwm = raw_channels[PPM_MAP_CH1];
    channel_2_pwm = raw_channels[PPM_MAP_CH2];
    channel_3_pwm = raw_channels[PPM_MAP_CH3];
    channel_4_pwm = raw_channels[PPM_MAP_CH4];
    channel_5_pwm = raw_channels[PPM_MAP_CH5];
    channel_6_pwm = raw_channels[PPM_MAP_CH6];
    
  #elif defined USE_PWM_RX
    // PWM: Read physical channels and apply mapping (PWM_MAP_CH1-6 map logical to physical pin channels)
    raw_channels[PWM_MAP_CH1] = getRadioPWM(PWM_MAP_CH1);
    raw_channels[PWM_MAP_CH2] = getRadioPWM(PWM_MAP_CH2);
    raw_channels[PWM_MAP_CH3] = getRadioPWM(PWM_MAP_CH3);
    raw_channels[PWM_MAP_CH4] = getRadioPWM(PWM_MAP_CH4);
    raw_channels[PWM_MAP_CH5] = getRadioPWM(PWM_MAP_CH5);
    raw_channels[PWM_MAP_CH6] = getRadioPWM(PWM_MAP_CH6);
    
    // Map to logical channels
    channel_1_pwm = raw_channels[PWM_MAP_CH1];
    channel_2_pwm = raw_channels[PWM_MAP_CH2];
    channel_3_pwm = raw_channels[PWM_MAP_CH3];
    channel_4_pwm = raw_channels[PWM_MAP_CH4];
    channel_5_pwm = raw_channels[PWM_MAP_CH5];
    channel_6_pwm = raw_channels[PWM_MAP_CH6];
    
  #elif defined USE_ELRS_RX
    // ELRS: Read physical CRSF channels and apply mapping (ELRS_MAP_CH1-6 map logical to physical CRSF channel indices)
    raw_channels[ELRS_MAP_CH1] = getRadioPWM(ELRS_MAP_CH1);
    raw_channels[ELRS_MAP_CH2] = getRadioPWM(ELRS_MAP_CH2);
    raw_channels[ELRS_MAP_CH3] = getRadioPWM(ELRS_MAP_CH3);
    raw_channels[ELRS_MAP_CH4] = getRadioPWM(ELRS_MAP_CH4);
    raw_channels[ELRS_MAP_CH5] = getRadioPWM(ELRS_MAP_CH5);
    raw_channels[ELRS_MAP_CH6] = getRadioPWM(ELRS_MAP_CH6);
    
    // Map to logical channels
    channel_1_pwm = raw_channels[ELRS_MAP_CH1];
    channel_2_pwm = raw_channels[ELRS_MAP_CH2];
    channel_3_pwm = raw_channels[ELRS_MAP_CH3];
    channel_4_pwm = raw_channels[ELRS_MAP_CH4];
    channel_5_pwm = raw_channels[ELRS_MAP_CH5];
    channel_6_pwm = raw_channels[ELRS_MAP_CH6];
    ...
    if (sbus.read(&sbusChannels[0], &sbusFailSafe, &sbusLostFrame))
    {
      ...
      // Read physical SBUS channels and apply mapping (SBUS_MAP_CH1-6 map logical to physical array indices, 0-based)
      raw_channels[SBUS_MAP_CH1] = sbusChannels[SBUS_MAP_CH1 - 1] * scale + bias; // Convert 1-based to 0-based
      raw_channels[SBUS_MAP_CH2] = sbusChannels[SBUS_MAP_CH2 - 1] * scale + bias;
      raw_channels[SBUS_MAP_CH3] = sbusChannels[SBUS_MAP_CH3 - 1] * scale + bias;
      raw_channels[SBUS_MAP_CH4] = sbusChannels[SBUS_MAP_CH4 - 1] * scale + bias;
      raw_channels[SBUS_MAP_CH5] = sbusChannels[SBUS_MAP_CH5 - 1] * scale + bias;
      raw_channels[SBUS_MAP_CH6] = sbusChannels[SBUS_MAP_CH6 - 1] * scale + bias;
      
      // Map to logical channels
      channel_1_pwm = raw_channels[SBUS_MAP_CH1];
      channel_2_pwm = raw_channels[SBUS_MAP_CH2];
      channel_3_pwm = raw_channels[SBUS_MAP_CH3];
      channel_4_pwm = raw_channels[SBUS_MAP_CH4];
      channel_5_pwm = raw_channels[SBUS_MAP_CH5];
      channel_6_pwm = raw_channels[SBUS_MAP_CH6];
    }
    ...
    if (DSM.timedOut(micros())) {
        ...
        #ifndef NUM_DSM_CHANNELS
          #define NUM_DSM_CHANNELS 6  // Default to 6 channels if not defined
        #endif
        uint16_t values[NUM_DSM_CHANNELS];
        DSM.getChannelValues(values, NUM_DSM_CHANNELS);

        // Read physical DSM channels and apply mapping (DSM_MAP_CH1-6 map logical to physical array indices, 0-based)
        raw_channels[DSM_MAP_CH1] = values[DSM_MAP_CH1 - 1]; // Convert 1-based to 0-based
        raw_channels[DSM_MAP_CH2] = values[DSM_MAP_CH2 - 1];
        raw_channels[DSM_MAP_CH3] = values[DSM_MAP_CH3 - 1];
        raw_channels[DSM_MAP_CH4] = values[DSM_MAP_CH4 - 1];
        raw_channels[DSM_MAP_CH5] = values[DSM_MAP_CH5 - 1];
        raw_channels[DSM_MAP_CH6] = values[DSM_MAP_CH6 - 1];
        
        // Map to logical channels
        channel_1_pwm = raw_channels[DSM_MAP_CH1];
        channel_2_pwm = raw_channels[DSM_MAP_CH2];
        channel_3_pwm = raw_channels[DSM_MAP_CH3];
